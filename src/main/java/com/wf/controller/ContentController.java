...
public class ContentController {
	...
	@ResponseBody
	public Object getMessageJson(SearchMessageParameter param, HttpServletResponse response,
		...
		try {
			MessageSearchRequest searchRequest = new MessageSearchRequest();
			searchRequest.setCount(param.getPageSize());
			searchRequest.setStartIndex((param.getPageNum() - 1) * param.getPageSize());

			searchRequest.setColums(param.getColums());
			searchRequest.setIssueState(param.getIssueState());
			searchRequest.setTitle(param.getTitle());
			searchRequest.setStartTime(param.getStartTime());
			searchRequest.setEndTime(param.getEndTime());
			searchRequest.setBranch(param.getBranch());
			searchRequest.setHuman(param.getHuman());
			PageList messageList = messageService.getMessage(searchRequest);
			messageList.setPageNum(param.getPageNum());
			messageList.setPageSize(param.getPageSize());
			//记录日志
			Log log = new Log("资讯管理", "查询", "查询条件:操作部门:" + param.getBranch() +
					",操作人:" + param.getHuman() + ",添加日期:" + param.getStartTime() + "-" + param.getEndTime()
					+ ",栏目:" + param.getColums() + ",栏目:" + param.getColums() + ",标题:" + param.getTitle()
	...
	public void updateMessageJson(Message message, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Wfadmin admin = CookieUtil.getWfadmin(request);
		message.setHuman(admin.getUser_realname());
		message.setBranch(admin.getDepartment());
...
//		}
	...
	public String hotword(Model model,HttpServletRequest request) {
		try {
			Map map = new HashMap();
			map.put("status", 1);
	...
	public Map UpdateInformationLabel(InformationLabelRequest request, HttpServletRequest req) {
		...
		try {
			...
			if (request.getTotalNumber() != null && !"".equals(request.getTotalNumber())) {
				searchRequest.setCount(request.getTotalNumber());
			}
			//查询标签在数据库是否存在
			SearchResponse<InformationLabel> searchResponse = informationLabelService.searchOnlyInformationLabel(searchRequest);
			if (searchResponse != null && searchResponse.getItems().size() > 0) {
