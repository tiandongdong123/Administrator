...
function sonpage(curr)
{
	...
	$.getJSON('../son/getson.do', {
		...
		for(var i =0;res.pageRow[i];i++){
			...
			if(purview.indexOf("F242")!=-1){
				html+="<td><button type='button' class='btn btn-primary' onclick=\"updateson("+res.pageRow[i].id+",'"+res.pageRow[i].sonName+"','"+res.pageRow[i].sonCode+"','"+res.pageRow[i].productResourceCode+"')\">修改</button>";
			}else{
				html+="<td><button style='display:none' type='button' class='btn btn-primary' onclick=\"updateson("+res.pageRow[i].id+",'"+res.pageRow[i].sonName+"','"+res.pageRow[i].sonCode+"','"+res.pageRow[i].productResourceCode+"')\">修改</button>";	
			}
			if(purview.indexOf("F243")!=-1){
				html+="<button type='button' class='btn btn-primary' onclick=\"deleteson("+res.pageRow[i].id+")\">删除</button></td></tr>";
			}else{
				html+="<button style='display:none' type='button' class='btn btn-primary' onclick=\"deleteson("+res.pageRow[i].id+")\">删除</button></td></tr>";
			}  
		}
