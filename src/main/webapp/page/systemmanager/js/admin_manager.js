...
function adminpage(curr){
    ...
    for(var i =0;res.pageRow[i];i++){
    	...
    	html+=" <tr><td><input type='checkbox' value='"+res.pageRow[i].id+"' name='ids'></td>" +
    			"<td>"+res.pageRow[i].wangfang_admin_id+"</td><td>"+res.pageRow[i].user_realname+"</td>" +
    			"<td>"+res.pageRow[i].dept.deptName+"</td><td>"+res.pageRow[i].mobile_phone+"</td>" +
    			"<td>"+res.pageRow[i].tel_phone+"</td><td>"+res.pageRow[i].email+"</td>" +
    			"<td>"+res.pageRow[i].role.roleName+"</td>"+status+"<td><button type='button' class='btn btn-primary' onclick=\"updateadmin('"+res.pageRow[i].id+"')\">修改</button>" +
